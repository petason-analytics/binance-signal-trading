...
async function bootstrap() {
  ...
  const app = await NestFactory.create(TradeModule, nestAppOpt);
  ...
  await app.listen(port).then(() => {
    logger.warn(`🚀 [Trade] Server ready at :${port} :${env} ${ssl ? ':ssl' : ''} 🚀`);
...
function onBootstrapped(app, logger) {
  // Sentry.test();
  Sentry.safeInit(() => {
    logger.log('[Trade] Sentry was enabled & initialized');
  });
}
